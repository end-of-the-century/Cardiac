...
def main():
    ...
    optimizer = optim.SGD(mynet_lv.parameters(),lr=1e-3, momentum=0.99)  #


    exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=2500, gamma=0.9)  #
    mynet_lv = nn.DataParallel(mynet_lv).cuda()


    # %% Train
    for t in range(num_epoch):  #

        mynet_lv.train()  # Set model to training mode
        ...
        if (t+1)%5==0:
            torch.save(mynet_lv, 'model/mynet_sobel_%d.pkl'%(t+1))
